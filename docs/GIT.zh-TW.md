...
另外還有**兩個呼叫者**,那也正是這條路徑不收面板 id 的原因。主控台上的 `n w` 從一個目錄、而不是從
一個 agent 出發,跑的是同一段程序;`A` 再按 `w` 則從 `A` 剛問到的工作目錄出發,帶著 `A` 已經挑好的
profile。見下方的[三個入口](#三個入口)。
...
同樣一棵樹、一個 agent、一個群組,有三個動詞可以到達;要用哪一個,取決於你此刻是不是已經在看某個
agent,以及 `A` 是不是已經選過後端。

| 動詞         | 在哪裡       | repo 來自           | agent profile        |
| ------------ | ------------ | ------------------- | -------------------- |
| `n w`        | 主控台       | 你自己挑的目錄      | **艦隊預設**         |
| `C-t G` `w`  | 放大的 agent | 該 agent 的工作目錄 | **該 agent** 的 spec |
| `A` then `w` | 主控台       | 你剛打的工作目錄    | **挑好的** profile   |
...
**`A` 再按 `w`** 是第三個。工作目錄之後,Baton 不是立刻生成,而是停住一次確認:enter 或 `n` 仍然
把 agent 開在**那個目錄裡**(這是預設,也是肌肉記憶);`w` 問分支,跑的是與 `n w` 相同的無目標
`panel.git` `worktree-add`,帶的是 `A` 已經挑好的 profile。取消會清掉那次挑選,免得它漏進下一次
生成。座艙不會去探這個目錄是不是 git 版本庫——遠端的艦隊甚至可能不在這台機器上,而一個非 repo
會由伺服器在狀態列拒絕,和 `n w` 一樣。

不是 git 版本庫的目錄會被**拒絕**:不生成、不建立任何目錄,也不會退回成一次普通的建立。同一個目錄裡
`A` 再按 enter,依然做它一向做的事——把 agent 開在版本庫**裡面**,不長出任何樹。

**`C-t G` `w`** 則是另一端:你正在看著一個 agent,而你想要另一個像它的 agent 待在自己的分支上。
它會複製那個 agent 的指令、參數與 profile,所以新的樹會拿到同一種 agent、落在同一組資源上限之下。

不論是哪一個動詞開出來的,關掉新面板都會讓樹留在原地。三個動詞都不移除任何東西;git 選單裡的 `x`
是一棵樹唯一的退場方式。
...
- **worktree-add** 解析出 repo 與 spec,再呼叫那條共用的 repo + 分支 + spec 路徑,由它建立這棵
  樹、把樹記下來、生出並分組該 agent,再廣播整個 fleet——沒有任何呼叫者留這段程序的副本。各呼叫者
  唯一的差別就在**怎麼解析**:有 `id` 時它指名一個面板,repo 與 spec 都從那裡來(`C-t G` `w`);而
  **空的 `id`** 是主控台那一式,由 `dir` 指名 repo、`path`/`args`/`profile` 帶著座艙已經解析好的
  spec——`n w` 用**艦隊預設**,`A` 再按 `w` 用**挑好的** profile。三個都送同一個命令,所以沒有新增
  第二個 wire action,協定版本也沒有動;只認得第一式的舊常駐程式,對空 id 那一式會回
  `no panel with id ""`——是拒絕,不是誤讀。對 conductor 連線,兩式都要付
  `panel.create` 的數量上限與速率上限,因為 worktree-add 是一次披著 git 操作名字的生成。只對無目標
  那一式收費等於什麼都沒擋:在數量上限被拒的 conductor,可以改走另一式把自己的 agent 一個分支一個
  分支扇出去,完全不計量。座艙一樣要付隊伍上限,只是付在生成的那一刻、而不是在門口。尋常的拒絕
  仍然來得比較早:worktreeSpawn 會在 git 執行前先讀那個上限,所以面對一支已經停在六十四的隊伍,
  請求會被擋下而不會留下任何樹。剩下的窗口是「隊伍在 git 執行期間才填滿」,那時樹確實已經存在,
  而回覆也會這樣說——它以 `worktree created at …` 開頭,並以那句數量上限的拒絕作結。先空出一個
  位子,或者用 `x` 把那棵樹收掉;
