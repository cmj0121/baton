...
`baton.spawn` 要付**隊伍上限**這道帳。六十四個面板就是六十四個,不管是誰來要第六十五個,
所以一個在迴圈裡開面板的 hook,會撞上和 conductor 同一道牆、收到同一段訊息。它**不必**付
開面板的速率上限,而 conductor 要付——這後半是一個刻意的保留,不是已經定案的答案。外掛是
無人看管、跑在迴圈裡的程式碼,而那正是速率上限存在的理由,同一套 API 裡的 `baton.enqueue`
也早就用 `queue.max` 計量了;之所以還沒開始計費,是因為一旦開了,就會在一個從來沒碰到過的
上限上,把一個原本好好的外掛弄壞。寫 hook 時請當作它日後可能會改,並且把隊伍上限當成真正
攔得住失控的那道限制。
...
| `score`   | 這次送達將要注入的 score 區塊,已針對它落地的那個面板排序;沒有時為空 |
| `cwd`     | 任務將落地的工作目錄                                                |
| `profile` | 目標面板的 agent profile                                            |
| `panel`   | 簡報將送達的面板 id                                                 |
...
| `false` / `{ drop = true }` | 否決——任務被丟棄;還在等待的呼叫者會收到錯誤,其餘的否決一律在待辦裡失敗                |

Table 裡的 `score` 值必須是字串;任何其他型別都會被忽略。Hook 會在這兩個欄位上一路串接
(較後的 hook 會同時看到前一個對 prompt 與 score 的改寫),而第一個否決會終止整條串接。
它只在**簡報送達某個面板的那一刻**執行,絕不提前——對 `dispatch` 或落在一個已就緒面板上的
`dispatch-group` 成員來說那就是下指令的當下;對其餘的簡報而言,則是面板安定下來、或排程器
把待辦排到面板上的那一刻。在簡報有面板之前,沒有東西可以拿來塑形它。外掛發起的派送會繞過它,
所以它永遠不會再次進入自己——包含 `baton.enqueue` 排進來的任務,無論它等了多久,送達時都是
原封不動的 prompt。

**派送到忙碌面板的簡報,會在 hook 看到它之前就先被回覆。**面板還在啟動或正在吐出輸出,簡報
因此等待,而 `baton ctl dispatch` 在整條串接尚未跑過時就回報成功。若 hook 之後否決了它,這個
否決會以排隊任務的方式抵達——任務帶著理由在待辦裡以 `failed` 收尾,並在 daemon 日誌留下一行
——而不是回到一條已經被回覆過的連線上。派送到已安定的面板,否決仍然是同步的。

上述每一個都是一次送往單一面板的送達,而簡報在整條串接跑起來之前就已經綁定到那個面板——
所以一個讓三個 agent 賽跑的 fan-out 會跑三次串接,每個成員一次,各自帶著那個成員自己的
`cwd`、`profile` 與 `score`。此處的否決只會丟掉那一個成員:唯有整個指令**一個面板都沒觸及**
時才會失敗——不論是每個成員都被拒絕,或是拒絕與下面說的略過加起來讓它一個都不剩;否則它會
成功,並回一則 notice,說明這次 fan-out 觸及了群組裡的幾個,以及其餘的為什麼沒收到。

一次 fan-out 在 `task.pre` 上最多花**兩秒**——就是單一 hook 拿到的同一份額度。額度是在每個成員
開始之前檢查,而不是在某個成員跑到一半時,所以最後一個開跑的成員仍然可以用掉它自己完整的逾時:
一個卡死的 hook 的最壞情況大約是四秒,而不是兩秒。超過額度之後,剩下
的成員**完全不會被派送**,而那則 notice 會**指名它們**,好讓你用 `panel.dispatch` 逐一送到,
那條路不受這個額度限制。不要單純把同一個 fan-out 再送一次:群組是以穩定順序走訪的,而每一個
指令都拿到全新的額度,所以切點每次都落在同一個位置——重送只會把簡報再送給已經收到的那些成員,
剩下的依然碰不到。

失敗時關閉是刻意的,而且刻意不同於旁邊那個逐次呼叫的逾時:在那裡,hook 是被問過而沒有及時
回答,所以它的簡報會不經過濾送出;而在這裡,剩下的 hook 根本沒被問過,派送沒有任何 hook 檢查
過的工作,會讓「可以拒絕」這件事失去意義——一個二十個成員的群組,搭配一個回答只要 100 毫秒的
hook,就已經足以碰到這個上限。觸及較少的面板、而且知道是哪些,是可以補救的;沒被檢查過的
工作則不行。
...
任何東西變慢時,一整批排隊的工作不會多花任何代價。另外,fan-out 會對每個成員各跑一次串接,
並且是在呼叫端的連線上跑:請讓 `task.pre` 保持快速,否則一個很寬的群組對發出指令的人來說就
會很慢,而且超過上面說的 fan-out 額度之後,它觸及的面板會比群組實際擁有的少。

`score` 欄位由 Score——整隊範圍的記憶——餵入
([#37](https://github.com/cmj0121/baton/issues/37),開發中);Score 關閉時,這個欄位就只是
...
Baton 不對它做沙盒隔離。若有需求,一個未來可選加入的受限模式(沒有 `os`/`io`、沒有網路)
是可能的,但預設是完整的力量。

正因為這個檔案被如此完全地信任,baton 會在執行前確認它真的是你的——第一次載入時檢查,
之後**每一次**重新載入也都再檢查一次,所以一個開機時還健全的檔案,一旦不再健全就會被拒絕。
它只問一個問題:有沒有**你以外的人**可能決定了被執行的內容?以下情況外掛會被拒絕:

- 它**可被群組或其他人寫入**(`0666`、`0664`……),
- 它**屬於另一個使用者**,或
- 它位於一個**可被群組或其他人寫入**且沒有 sticky bit 的**目錄**——在那裡,檔案自己的 `0600`
  只是裝飾,因為任何能 unlink 它的人都能放自己的檔案取而代之。共用的 `/tmp` 有 sticky bit,
  因此沒有問題。

有兩件事它刻意**不做**。它不介意別人能**讀取**這個檔案:`0644` 正是從 dotfiles repository
取出的外掛會有的模式,而可讀不等於可寫。它也會**跟隨 symlink** 而不是拒絕它,因為把
`~/.baton/plug-in.lua` 指向一個 repository 是保存外掛的常見做法——兩端都會檢查,包括
link 自己的目錄,以及目標檔案與它的目錄。

拒絕是**大聲的,但不致命**。常駐程式會繼續執行(重新載入發生時你的 agent 正在運作,為了一個
模式位元而失去整個機隊是更糟的交換),但和 Lua 語法錯誤不同,它會以 error 等級記錄,**並且**
在每個已連上的 cockpit 發出通知。語法錯誤是你自己的檔案在你造成它的一個按鍵之後失敗;而一個
運作了好幾個月、卻在某個模式位元被改動後停止的外掛,看起來和一個正常運作的外掛一模一樣,
所以它會在你看得到的地方說出來。修正方式:

```sh
chmod 600 ~/.baton/plug-in.lua
```

`--plugin FILE` 可以指向任何地方,所以這些檢查是針對檔案本身而不是它的位置;`$HOME/.baton`
是預設值,不是規則。
