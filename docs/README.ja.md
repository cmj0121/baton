...
1. **`A`** を押して agent を起動します(作業ディレクトリを選び、enter でここに置き、`w` でブランチに隔離します)。
...
tmux は、この話の半分にしか当たりません。herdr、Claude Squad、cmux は、すでに pane の中身が agent だと前提を置い
ています。同じ製品の着替えではありません:

|                                   | Baton                       | herdr              | Claude Squad     | cmux                       |
| --------------------------------- | --------------------------- | ------------------ | ---------------- | -------------------------- |
| 実行系                            | 自前の PTY daemon           | 自前の PTY daemon  | tmux のラップ    | GPU 端末                   |
| 誰が待っているか                  | `C-t a` の受信箱            | blocked サイドバー | セッション一覧   | notification rings         |
| 隔離                              | `A` での提案;第一級の `n w` | workspaces         | spawn = worktree | workspaces                 |
| Score・タスクキュー・上限・使用量 | Baton だけ                  | —                  | —                | —                          |
| ライセンス                        | `MIT`                       | `Apache 2.0`       | `AGPL-3.0`       | `GPL-3.0-or-later`         |
| 対 tmux                           | 置き換えではない            | 置き換えの形       | それを包む       | 端末であって tmux ではない |

Score、タスクキュー、プロセスツリーの上限、使用量フッタは、他の三つには対になるものがありません。
...
|                | `p` / `n c`       | 新しい shell / command panel                                        |
|                | `A`               | 新しい agent — 作業ディレクトリのあと、enter でここか `w` で隔離    |
|                | `n w`             | 先に隔離:リポジトリ、それからブランチ、fleet 既定の agent           |
...
|                | `g g` / `g c`     | マーク / マークした panel をまとめる                                |
|                | `g a` / `g u`     | 選択中の work item に追加 / 解除                                    |
|                | `s` / `f` / `D`   | 選択中にシグナル / 検索 / diff                                      |
|                | `/`               | すべての panel の出力を検索(fleet を grep)                          |
|                | `T` / `Q` / `I`   | タスクを投げる / キュー / GitHub issues                             |
