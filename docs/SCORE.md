...
**From the cockpit, without editing the file:** `C-t P` opens panel config; `←→` walks its tabs to **FEEDBACK**, which
lists every configured profile — `↑↓` to move, `e` to cycle that profile through `inherit → on → off → inherit`. The
