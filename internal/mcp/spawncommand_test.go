...
// TestMCPSpawnCommand is the conductor's half of #54. `command` is an extra field
...
//
// The two refusals are the point of the field. `agent` and `command` together is
...
// exactly the slip a model makes. `worktree` with `command` has no meaning at all:
...
func TestMCPSpawnCommand(t *testing.T) {
	...
	resps := run(t, sock,
		fmt.Sprintf(`{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"baton_spawn","arguments":{"command":"/bin/sh","args":["-c","sleep 30"],"dir":%q}}}`, dir),
		`{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"baton_spawn","arguments":{"agent":"/bin/sh","command":"/bin/sh"}}}`,
		fmt.Sprintf(`{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"baton_spawn","arguments":{"command":"/bin/sh","worktree":true,"branch":"feat/x","dir":%q}}}`, dir),
	...
	if resps[1].Result["isError"] != true {
		t.Fatalf("agent with command should be a tool error, got %+v", resps[1].Result)
	}
	...
	if resps[2].Result["isError"] != true {
		t.Fatalf("command with worktree should be a tool error, got %+v", resps[2].Result)
	}
