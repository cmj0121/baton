...
func TestScheduleAssignsToIdleAgent(t *testing.T) {
	...
	id, err := s.enqueueTask(conn(""), "do it", "api", nil)
	if err != nil || id != "t1" {
...
func TestScheduleHonoursConcurrencyCap(t *testing.T) {
	...
	if _, err := s.enqueueTask(conn(""), "a", "api", nil); err != nil {
		t.Fatalf("enqueue a: %v", err)
	}
	if _, err := s.enqueueTask(conn(""), "b", "api", nil); err != nil {
...
func TestScheduleRespectsGroup(t *testing.T) {
	...
	if _, err := s.enqueueTask(conn(""), "api work", "api", nil); err != nil {
...
func TestQueueMaxRejectsOverflow(t *testing.T) {
	s, _, _ := gateServer()
	s.queueMax = 1
	if _, err := s.enqueueTask(conn(""), "a", "", nil); err != nil {
		t.Fatalf("first enqueue should fit: %v", err)
	}
	if _, err := s.enqueueTask(conn(""), "b", "", nil); err == nil {
...
func TestCancelAndDrain(t *testing.T) {
	...
	a, _ := s.enqueueTask(conn(""), "a", "", nil)
	b, _ := s.enqueueTask(conn(""), "b", "", nil)
	c, _ := s.enqueueTask(conn(""), "c", "", nil)
